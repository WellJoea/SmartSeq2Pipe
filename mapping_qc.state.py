...
class Mapping():
    def __init__(self,args):
        self.args = args
        self.IDs = [ os.path.basename(i) for i in glob.glob(self.args.input + '/*') if os.path.exists(i + '/STAR') ]
    def starstate(self, Sid):
        starfile = '%s/%s/STAR/%s.Log.final.out' %(self.args.input, Sid, Sid)
        try:
...
class RSEMce():
    ...
    def unitpm(self, Sid, redup='Max'):
        rsemgene = '%s/%s/RSEM/%s.rsemce.genes.results' %(self.args.input, Sid, Sid)
